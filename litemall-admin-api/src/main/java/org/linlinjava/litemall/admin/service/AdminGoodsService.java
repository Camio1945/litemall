...
public class AdminGoodsService {
  ...
  @Autowired private LitemallGoodsSpecificationService specificationService;
  @Autowired private LitemallGoodsAttributeService attributeService;
  ...
  public Object list(
    ...
    List<LitemallGoods> goodsList =
        goodsService.querySelective(goodsId, goodsSn, name, page, limit, sort, order);
   ...
   * 编辑商品
   *
   * <p>NOTE： 由于商品涉及到四个表，特别是litemall_goods_product表依赖litemall_goods_specification表，
   * 这导致允许所有字段都是可编辑会带来一些问题，因此这里商品编辑功能是受限制： （1）litemall_goods表可以编辑字段；
   * （2）litemall_goods_specification表只能编辑pic_url字段，其他操作不支持； （3）litemall_goods_product表只能编辑price,
   * number和url字段，其他操作不支持； （4）litemall_goods_attribute表支持编辑、添加和删除操作。
   *
   * <p>NOTE2: 前后端这里使用了一个小技巧： 如果前端传来的update_time字段是空，则说明前端已经更新了某个记录，则这个记录会更新； 否则说明这个记录没有编辑过，无需更新该记录。
   *
   * <p>NOTE3: （1）购物车缓存了一些商品信息，因此需要及时更新。 目前这些字段是goods_sn, goods_name, price, pic_url。
  ...
  public Object update(GoodsAllinone goodsAllinone) {
    ...
    // 将生成的分享图片地址写入数据库
    String url =
        qCodeService.createGoodShareImage(
            goods.getId().toString(), goods.getPicUrl(), goods.getName());
